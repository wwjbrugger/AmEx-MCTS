...
class BestFirstSearch:
    ...
    def run_search(self, state: GymGameState, num_mcts_sims) -> typing.Tuple[np.ndarray, float]:
        ...
        # insert all actions available in root into the priority queue
        moves = self.game.getLegalMoves(state).astype(bool)
        for action in range(len(moves)):
            if moves[action]:
                child, r = self.game.getNextState(state, action)
                if child.hash not in S:
                    S.append(child.hash)
                    v = self.simulate(child)
                    priority_queue.put(PrioritizedItem(v + r, child))

        num_children = len(S)
        for num_sim in range(num_mcts_sims - num_children):
            # select
            ...
            q[node.production_action] = max(v, q[node.production_action])
